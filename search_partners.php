...
<head>
    ...
    <title>การจัดการข้อมูลคู่ค้า</title>
...
<body>
    ...
    <div class="container">
        ...
        $sql = "SELECT * FROM tb_partners WHERE partners_fname LIKE '%$partners_data%' OR partners_lname LIKE '%$partners_data%' ORDER BY partners_fname ASC ";
        $result = mysqli_query($con, $sql);
        $count = mysqli_num_rows($result);
        $order = 1;
        ?>
        <br>
        <h3 class="text-center">การจัดการข้อมูลคู่ค้า</h3>
        <br>
        <div class="btn-add">
            <a type="button" class="btn btn-success" href="insert_partners.php">
        ...
        <div class="col-md-7">
            <form action="search_partners.php" method="POST">
                <div class="input-group mb-3">
                    <input type="text" name="partners_data" required class="form-control" placeholder="กรอกชื่อที่ต้องการจะค้นหา...">
        ...
        <?php if ($count > 0) { ?>
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    ...
                    include('condb.php');
                    $query = "
                        SELECT p.*,n.prefix_name
                        FROM tb_partners as p
                        INNER JOIN  tb_nameprefix as n ON p.ref_nameprefix_number = n.prefix_number
                        ORDER BY n.prefix_number asc";
                    $result = mysqli_query($con, $query);
                    while ($row = mysqli_fetch_assoc($result)) {
                    ?>
                        <tr>
                            <td><?php echo $row["partners_number"]; ?></td>
                            <td><?php echo $row["partners_id"]; ?></td>
                            <td><?php echo $row["prefix_name"]; ?></td>
                            <td><?php echo $row["partners_fname"]; ?></td>
                            <td><?php echo $row["partners_lname"]; ?></td>
                            <td><?php echo $row["partners_phone"]; ?></td>
                            <td><?php echo $row["partners_company"]; ?></td>
                            <td>
                                <a href="update_partners.php?partners_number=<?php echo $row['partners_number']; ?>" class="btn btn-primary"><iconify-icon icon="el:file-edit"></iconify-icon></a>
                                <a onclick="return confirm('คุณแน่ใจหรือว่าต้องการลบรายการนี้?')" href="delete_partners.php?partners_number=<?php echo $row['partners_number']; ?>" class='btn btn-danger remove'><iconify-icon icon="ant-design:delete-outlined"></iconify-icon></a>
                            </td>
                        </tr>
                    <?php } ?>
                </tbody>
        ...
        <?php } else { ?>
            <div class="alert alert-danger">
                <b>ไม่พบข้อมูลคู่ค้า!!</b>
            </div>
        <?php } ?>
        <a href="partners.php" class="btn btn-success">ย้อนกลับ</a>
