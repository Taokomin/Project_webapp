...
if (!$_SESSION["UserID"]) {
    ...
    <head>
        ...
        <title>การจัดการข้อมูลการรับคืนวัสดุและอุปกรณ์</title>
    ...
    <body>
        ...
        <div class="container">
            <br>
            <h3 class="text-center">การจัดการข้อมูลการรับคืนวัสดุและอุปกรณ์</h3>
            <br>
            <div class="btn-add">
                <a type="button" class="btn btn-success" href="insert_takeback.php">
                    <iconify-icon icon="carbon:document-add" style="color: white;" width="42" height="42"></iconify-icon>
                </a>
            </div>
            <br>
            <div class="col-md-7">
                <form action="search_takeback.php" method="POST">
                    <div class="input-group mb-3">
                        <input type="text" name="takeback_data" required class="form-control" placeholder="กรอกชื่อที่ต้องการจะค้นหา...">
            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    ...
                    // ";
                    $query = "SELECT * FROM tb_takeback  ORDER BY takeback_number asc" or die("Error:" . mysqli_error());
                    //WHERE ref_pickup_material_status = 'อนุมัติ'
                    $result = mysqli_query($con, $query);
                    while ($values = mysqli_fetch_assoc($result)) {
                    ?>
                        <tr>
                            <td><?php echo $values["takeback_number"]; ?></td>
                            <td><?php echo $values["takeback_id"]; ?></td>
                            <td><?php echo $values["takeback_day"]; ?></td>
                            ...
                            <td>
                                <a href="update_takeback.php?takeback_number=<?php echo $values['takeback_number']; ?>" class="btn btn-primary"><iconify-icon icon="el:file-edit"></iconify-icon></a>
                                <a onclick="return confirm('คุณแน่ใจหรือว่าต้องการลบรายการนี้?')" href="delete_takeback.php?takeback_number=<?php echo $values['takeback_number']; ?>" class='btn btn-danger remove'><iconify-icon icon="ant-design:delete-outlined"></iconify-icon></a>
                            </td>
                        </tr>
                    <?php
                    }
