...
if (!$_SESSION["UserID"]) {
    ...
    <head>
        ...
        <title>การจัดการข้อมูลหน่วยนับ</title>
    ...
    <body>
        ...
        <div class="container">
            <br>
            <h3 class="text-center">การจัดการข้อมูลหน่วยนับ</h3>
            <br>
            <div class="btn-add">
            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    <?php
                    include('condb.php');
                    $query = "SELECT * FROM tb_unit ORDER BY unit_number asc" or die("Error:" . mysqli_error());
                    $result = mysqli_query($con, $query);
                    while ($values = mysqli_fetch_assoc($result)) {
                    ?>
                        <tr>
                            <td><?php echo $values["unit_number"]; ?></td>
                            <td><?php echo $values["unit_id"]; ?></td>
                            <td><?php echo $values["unit_name"]; ?></td>
                            <td>
                                <a href="update_unit.php?unit_number=<?php echo $values['unit_number']; ?>" class="btn btn-primary"><iconify-icon icon="el:file-edit"></iconify-icon></a>
                                <a onclick="return confirm('คุณแน่ใจหรือว่าต้องการลบรายการนี้?')" href="delete_unit.php?unit_number=<?php echo $values['unit_number']; ?>" class='btn btn-danger'><iconify-icon icon="ant-design:delete-outlined"></iconify-icon></a>
                            </td>
                        </tr>
                    <?php
                    }
