...
$GLOBALS['equipment_number'] = '0';

$sql = "SELECT equipment_id FROM tb_equipment ORDER BY equipment_id DESC LIMIT 1";
$query = $con->query($sql);
$result = $query->fetch_assoc();

if ($result['equipment_id']) {
    $GLOBALS['equipment_id'] = $result['equipment_id'];
}

function increaseId($equipment_id)
{
    $matchId = preg_replace('/[^0-9]/', '', $equipment_id);
...
<body>
    ...
    <div class="container">
        ...
        <hr>
        <form action="insert_equipment_db.php" method="post">
            <div class="mb-3">
                <label for="equipment_id" class="form-label">รหัสพนักงาน</label>
                <input type="text" class="form-control" name="equipment_id" value="<?php echo (increaseId($GLOBALS['equipment_id'])); ?>" readonly>
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" name="equipment_quantity" required>
            </div>
            <div class="mb-3">
